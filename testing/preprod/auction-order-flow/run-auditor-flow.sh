...
wait_tx_info() {
  ...
  echo "Timed out waiting for tx $tx_hash in Koios" >&2
  return 1
}
...
if [[ -z "$NODE_SOCKET_PATH" && -t 0 ]]; then
...
set_env_value AUCTION_STEP_LEN_SECS "${AUCTION_STEP_LEN_SECS:-600}"
set_env_value MIN_SPAN_REMAINING_SECS "${MIN_SPAN_REMAINING_SECS:-120}"
