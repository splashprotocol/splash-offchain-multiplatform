...
impl RefScriptsOutputs {
    pub async fn new<'a>(config: RefScriptsConfig, explorer: Explorer<'a>) -> Option<RefScriptsOutputs> {
        async fn process_utxo_with_ref_script<'a>(tx_out: OutputRef, raw_ref_script: &str, explorer: Explorer<'a>) -> Option<TransactionUnspentOutput> {
            let previous_output: TransactionUnspentOutput = explorer.get_utxo(tx_out)
                .await
                .unwrap()
                .try_into()
                .ok()?;
            ...
            Some(TransactionUnspentOutput::new(previous_output.input, updated_new_output))
        }

        let pool_v1 = process_utxo_with_ref_script(
            OutputRef::from(config.pool_v1_ref), POOL_V1_SCRIPT, explorer,
        ).await?;
        let pool_v2 = process_utxo_with_ref_script(
            OutputRef::from(config.pool_v2_ref), POOL_V2_SCRIPT, explorer,
        ).await?;
        let swap = process_utxo_with_ref_script(
            OutputRef::from(config.swap_ref), SWAP_SCRIPT, explorer,
        ).await?;
        let deposit = process_utxo_with_ref_script(
            OutputRef::from(config.deposit_ref), DEPOSIT_SCRIPT, explorer,
        ).await?;
        let redeem = process_utxo_with_ref_script(
            OutputRef::from(config.redeem_ref), REDEEM_SCRIPT, explorer,
        ).await?;
