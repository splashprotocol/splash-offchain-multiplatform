...
impl Pool for StablePoolT2T {
    ...
    fn static_price(&self) -> StaticPrice {
        ...
        let price_denom = self.treasury_fee.denom().to_u64().unwrap()
            * (ann + (dn1 / U512::from(nn).mul(U512::from(y_calc).pow(U512::from(2))).mul(U512::from(x_calc))).as_u64());
    ...
    fn swap(mut self, input: Side<u64>) -> (u64, Self) {
        ...
        let treasury_fee = if valid_treasury_fee || *self.treasury_fee.numer() == 0 {
            ...
            while !valid_treasury_fee {
                println!("treasury_fee_ {:?}", treasury_fee_);
                println!("LEFT {:?}", treasury_fee_ * f_rev);
                println!("RIGHT {:?}", (quote_total_delta) * self.treasury_fee.numer());
