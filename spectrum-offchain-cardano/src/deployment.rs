...
impl Display for DeployedValidatorErased {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.write_str(format!("DeployedValidatorErased([reference_utxo: {}#{}, hash {})", self.reference_utxo.input.transaction_id, self.reference_utxo.input.index, self.hash).as_str())
    }
}
